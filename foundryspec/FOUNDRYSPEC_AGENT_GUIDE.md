...
## 📝 Footnotes and Deep Documentation
For the "Zero-Question Implementation" goal, Mermaid diagrams often need more detail than a node label can provide.
- **Interactivity**: Use `click NodeID "/footnotes/CATEGORY/NodeId.md"` in your Mermaid files.
- **Storage**: Store individual markdown files in `assets/CATEGORY/footnotes/NodeId.md`.
- **Centralization**: The build engine moves these to `footnotes/CATEGORY/*.md` for clean server-side resolution.
- **Discovery**: When reading existing specs, check the `footnotes/` directory within each category.
