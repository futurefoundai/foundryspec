...
1.  **Locate Requirements:**
    ...
    - _Check:_ Is the requirement linked to a component?
...
2.  **Verify Architecture:**
    ...
    - _Rule:_ If the diagrams are missing, vague, or use generic names (e.g., "Service", "Manager"), **STOP**. Switch to the `design-feature` workflow to complete the spec.
...
3.  **Traceability Mapping:**
    - Add `@foundryspec ID` markers to the top of every new class/module.
    - Add `@foundryspec REQUIREMENT REQ_ID` markers to functions that implement specific logic.

# Phase 4: Verification (The Loop)

**Objective:** Ensure code and spec remain in sync.

1.  **Semantic Traceability Check:**

    - Run `foundryspec build`.
    - _Check:_ Does the build pass with no "Semantic Errors"?
    - _Check:_ Open the hub and verify that the requirement node correctly maps to your implementation location.
